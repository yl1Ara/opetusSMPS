...
measurement_generation_lock = threading.Lock()
measurement_step_lock = threading.Lock()
measurement_maintenance_fd = None
init_maintenance_fd = None
tuning_maintenance_fd = None
calibration_maintenance_fd = None
...
RUNTIME_BRIDGE_KEY = "tdmps_gui_runtime_bridge"
RUNTIME_BRIDGE_LOCK_KEY = "tdmps_gui_runtime_bridge_lock"
app_stop_event = threading.Event()
runtime_bridge_lock = pn.state.cache.setdefault(
    RUNTIME_BRIDGE_LOCK_KEY, threading.Lock(),
)
with runtime_bridge_lock:
    runtime_bridge = pn.state.cache.get(RUNTIME_BRIDGE_KEY)
    runtime_owner = runtime_bridge is None
    if runtime_owner:
        runtime_bridge = {
            "ready": False,
            "owner_id": uuid.uuid4().hex,
            "lock": threading.Lock(),
            "owner_document": pn.state.curdoc,
            "request": None,
            "snapshot": None,
        }
        pn.state.cache[RUNTIME_BRIDGE_KEY] = runtime_bridge
...
def set_status_threadsafe(text):
    doc = runtime_bridge.get("owner_document") if runtime_owner else pn.state.curdoc
    if doc is not None:
...
def set_hv_for_point(point, generation):
    global hv_target_voltage, hv_runtime_status
    with hv_io_lock:
        if (
            app_stop_event.is_set()
            or not measurement_running.is_set()
            or generation != current_measurement_generation()
        ):
            return
        dp = point["dp"]
...
def init_done_callback(fut, start_after=False):
    global init_running, init_maintenance_fd
    import traceback as _traceback
    ...
    try:
        ...
        release_maintenance_lease(init_maintenance_fd)
        init_maintenance_fd = None
        init_running = False
...
def init(start_after=False):
    ...
    if hardware_stop_pending:
        status_text.object = "Status: hardware initialization refused until HV stop/zero finishes"
        return
    if init_running:
    ...
    if lease is None:
        status_text.object = "Status: hardware initialization refused during software update"
        return
...
def run_sheath_tuning_blocking(config):
    global pending_tuning_result, tuning_maintenance_fd
    ...
    try:
        ...
        release_maintenance_lease(tuning_maintenance_fd)
        tuning_maintenance_fd = None
...
def start_sheath_tuning(event=None):
    ...
    if tuning_running.is_set():
        return
    tuning_maintenance_fd = acquire_maintenance_lease()
    if tuning_maintenance_fd is None:
        sheath_tune_status.object = "Sheath tuning refused: software update is in progress"
        return
...
def apply_sheath_tuning_result(event=None):
    ...
    lease = acquire_maintenance_lease()
    if lease is None:
        sheath_tune_status.object = "Apply refused: software update is in progress"
        return
    try:
...
def run_aerosol_calibration_blocking(meter, settings, sample_count):
    global aerosol_active_config, calibration_maintenance_fd
    ...
    try:
        ...
        release_maintenance_lease(calibration_maintenance_fd)
        calibration_maintenance_fd = None
...
def start_aerosol_calibration(event=None):
    global calibration_maintenance_fd
    ...
    if calibration_running.is_set():
        return
    calibration_maintenance_fd = acquire_maintenance_lease()
    if calibration_maintenance_fd is None:
        aerosol_calibration_status.object = "Calibration refused: software update is in progress"
        return
...
def build_health_payload():
    ...
    elif hardware_stop_pending:
        runtime_state = "stopping"
    elif init_running:
        runtime_state = "initializing"
    elif measurement_running.is_set():
        runtime_state = "running"
    elif tuning_running.is_set():
        runtime_state = "tuning"
    elif calibration_running.is_set():
        runtime_state = "calibration"
    else:
        runtime_state = "idle"
    return {
        **software_identity(),
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "runtime_id": runtime_bridge["owner_id"],
        "pid": os.getpid(),
...
def _safe_shutdown_impl(reason):
    ...
    for thread in (
        measurement_thread, cpc_reader_thread, aerosol_poll_thread, health_thread,
    ):
    ...
    blower = None
    release_measurement_maintenance_lock()
    for lease in (init_maintenance_fd, tuning_maintenance_fd, calibration_maintenance_fd):
        release_maintenance_lease(lease)
    init_maintenance_fd = tuning_maintenance_fd = calibration_maintenance_fd = None
    if errors:
...
def current_measurement_generation():
    with measurement_generation_lock:
        return measurement_generation


def advance_measurement_generation():
    global measurement_generation
    with measurement_generation_lock:
...
def acquire_maintenance_lease():
    path = STATE_DIR / "maintenance.lock"
...
def release_maintenance_lease(fd):
    if fd is not None:
        os.close(fd)


def acquire_measurement_maintenance_lock():
    global measurement_maintenance_fd

    if measurement_maintenance_fd is not None:
        return True
    fd = acquire_maintenance_lease()
    if fd is None:
        return False
    measurement_maintenance_fd = fd
    return True


def release_measurement_maintenance_lock():
    global measurement_maintenance_fd

    if measurement_maintenance_fd is not None:
        os.close(measurement_maintenance_fd)
        measurement_maintenance_fd = None


def apply_scan_point(point, generation):
    ...
    set_hv_for_point(point, generation)
...
def measurement_step(generation, debug=True):
    import traceback as _traceback
    ...
    if not start_button.value or generation != current_measurement_generation():
        return
    ...
    try:
        ...
        if phase == "measuring":
            ...
            if measurement_finished:
                ...
                apply_scan_point(point, generation)
...
def startup_load():
    global runtime_sync_callback, last_scan_saved
    ...
    if runtime_owner:
        ...
        sync_runtime_owner_view()
        if runtime_sync_callback is None:
            runtime_sync_callback = pn.state.add_periodic_callback(
                sync_runtime_owner_view, period=500, start=True,
            )
...
def runtime_snapshot():
    return {
        ...
        "settings": {
            ...
            for widget in scan_setting_widgets
        },
    }
...
def publish_runtime_snapshot():
    ...
    with runtime_bridge["lock"]:
        runtime_bridge["snapshot"] = snapshot
...
def run_runtime_command(command, payload=None):
    try:
        if command == "start" and not start_button.value:
            ...
            if widget is not None and not widget.disabled:
                widget.value = value
        publish_runtime_snapshot()
    except Exception as error:
        record_runtime_error(error)
        status_text.object = f"Status: remote command failed: {error}"
        publish_runtime_snapshot()


def queue_runtime_command(command, payload=None):
    owner_document = runtime_bridge["owner_document"]
    owner_document.add_next_tick_callback(
        lambda: run_runtime_command(str(command), payload),
    )


runtime_syncing = False
runtime_snapshot_signature = None


def sync_runtime_owner_view():
    global runtime_syncing, runtime_snapshot_signature
    with runtime_bridge["lock"]:
        snapshot = runtime_bridge.get("snapshot")
    if not runtime_bridge.get("ready") or snapshot is None:
        status_text.object = "Status: instrument runtime is initializing"
        return

    runtime_syncing = True
    try:
        start_button.value = bool(snapshot["scan_active"])
        status_text.object = str(snapshot["status"])
        scan_progress_pane.object = str(snapshot["scan_progress"])
        pending_settings_pane.object = str(snapshot["pending_settings"])
        ...
        for widget_name, setting in snapshot["settings"].items():
            ...
            if widget is not None:
                widget.value = setting["value"]
                widget.disabled = bool(setting["disabled"])
        qc_table.value = snapshot["qc"].copy()
        table = snapshot["table"].copy()
        signature = (
            len(table),
            str(table.iloc[-1].get("time", "")) if len(table) else "",
        )
        if signature != runtime_snapshot_signature:
            runtime_snapshot_signature = signature
            table_pane.value = table
            refresh_live_plot(force=True)
    finally:
        runtime_syncing = False


def on_runtime_follower_start_change(event):
    if runtime_syncing:
        return
    send_runtime_command("start" if event.new else "stop")


def on_runtime_follower_setting_change(event, widget_name):
    if runtime_syncing:
        return
...
def send_runtime_command(command, payload=None):
    request = runtime_bridge.get("request")
    if request is None:
        status_text.object = "Status: instrument runtime is not ready"
        return
...
if runtime_owner:
    ...
    for widget in scan_setting_widgets:
        widget.param.watch(
            lambda event, name=widget.name: on_runtime_follower_setting_change(event, name),
            "value",
        )
...
if runtime_owner:
    ...
    runtime_bridge.update({
        "ready": True,
        "request": queue_runtime_command,
    })
    owner_document = runtime_bridge["owner_document"]
    ui_callback = owner_document.add_periodic_callback(drain_ui_updates, 500)
    owner_document.add_periodic_callback(publish_runtime_snapshot, 500)
...
def runtime_owner_session_destroyed(session_context):
    if not runtime_owner:
        return
    owner_document = runtime_bridge["owner_document"]
    owner_context = owner_document.session_context
    if owner_context is None or session_context.id != owner_context.id:
        return
    with runtime_bridge_lock:
        if pn.state.cache.get(RUNTIME_BRIDGE_KEY) is not runtime_bridge:
            return
        safe_shutdown("runtime owner session expired")
        atexit.unregister(safe_shutdown)
        release_owner = runtime_bridge.get("release_owner")
        if release_owner is not None:
            release_owner()
        pn.state.cache.pop(RUNTIME_BRIDGE_KEY, None)
...
if runtime_owner and threading.current_thread() is threading.main_thread():
